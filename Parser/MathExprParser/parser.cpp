...
//        user_var.add("Ans", ans);
        ...
        snprintf(ans_str, sizeof(ans_str), "%.15g", ans);
    }
    catch (Error err)
    {
        if (err.get_row() == -1)
...
double Parser::eval_operator(const int op_id, const double &lhs, const double &rhs)
{
    switch (op_id)
    {
        ...
        case MODULUS:   return static_cast<int>(lhs) % static_cast<int>(rhs); //give a warning if the values are not integer
...
bool Parser::ifVarExist(const char * name){
	return user_var.exist(name);
}
